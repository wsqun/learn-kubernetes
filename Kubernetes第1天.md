...
![image-20200524150339551.png](assets/image-20200524150339551.png)

## kubernetes简介

![image-20200406232838722.png](assets/image-20200406232838722.png)
...
![image-20200406184656917.png](assets/image-20200406184656917.png)
...
![image-20200404094800622](assets/image-20200403224313355.png)
...
#### 问题
如果还是NoReady,可使用`journalctl -f -u kubelet`查看报错信息，
```
12月 11 10:04:00 node1 kubelet[4347]: W1211 10:04:00.048822    4347 cni.go:237] Unable to update cni config: no networks found in /etc/cni/net.d
12月 11 10:04:01 node1 kubelet[4347]: E1211 10:04:01.195385    4347 kubelet.go:2183] Container runtime network not ready: NetworkReady=false reason:NetworkPluginNotReady message:docker: network plugin is not ready: cni config uninitialized
```
安装网络插件是在master进行，会出现node节点没同步插件配置，可将master的配置`/etc/cni/net.d`复制到node，然后进行重启`systemctl restart kubelet`
...
![image-20200405142656921.png](assets/image-20200405142656921.png)
...
kubernetes/pause    latest              f9d5de079539        7 years ago         240kB

// 设置tag
[root@node2 ~]# docker tag kubernetes/pause:latest k8s.gcr.io/pause:3.1

[root@node2 ~]# docker images
REPOSITORY          TAG                 IMAGE ID            CREATED             SIZE
kubernetes/pause    latest              f9d5de079539        7 years ago         240kB
k8s.gcr.io/pause    3.1                 f9d5de079539        7 years ago         240kB
```
...
![image-20200406225334627.png](assets/image-20200406225334627.png)
